...
description: 'Build polished, conversion-aware frontends with strong visual taste:
  landing pages, dashboards, components, emails, redesigns.'
metadata:
  openclaw:
    emoji: 🎨
    requires:
...
> **Workspace path:** Throughout this skill, `<WORKSPACE>` is the agent's workspace directory. Resolve it at runtime: `$OPENCLAW_WORKSPACE` if set, else `agents.defaults.workspace` in `~/.openclaw/openclaw.json`, else the current default `~/.openclaw/workspace`. (Older installs may have used `~/workspace` — do not assume it.)
...
If attaching via Discord, attach the generated index.html from the designs directory directly.
...
### Discord
Attach the generated index.html from the designs directory directly.
